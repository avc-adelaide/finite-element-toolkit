...
The \textbf{Finite Element Method} (FEM) is a powerful and
widely used numerical technique for solving partial differential equations
(PDEs) arising in science and engineering.
\textbf{FEniCSx} is an open-source computing platform for solving PDEs
using the finite element method~\cite{LoggMardalWells2012, Scroggs2022}, which
provides high-level abstractions that allow users to express variational
...
underlying the finite element method, and their mapping to FEniCSx concepts and terminology.
...
\begin{itemize}
  \item $\domain \subset \R^d$ denotes an open, bounded domain with
    ...
    \footnote{%
    Lipschitz means the boundary is locally representable as a graph with bounded (i.e., non-infinite) slope, excluding cusps and fractal irregularities.
    Note that this use of the $\partial$ symbol is unrelated to a partial derivative. This notation is standard in topology, differential geometry, and differential topology.}
...
\emph{well-posed} in the sense of Hadamard: existence, uniqueness, and continuous dependence on
data:
\begin{itemize}
  \item \textbf{Existence:} There exists a solution \(u\) that satisfies the PDE and the prescribed boundary/initial conditions.
  \item \textbf{Uniqueness:} If \(u_1\) and \(u_2\) both satisfy the PDE with the same data, then \(u_1 = u_2\).
  \item \textbf{Continuous dependence on data:} Small changes in the boundary/initial data lead to small changes in the solution (in an appropriate norm).
\end{itemize}
...
We must specify appropriate boundary conditions that model how the exterior influences the domain.
Boundary conditions can be one of thee types: Dirichlet, Neumann, or Robin.
...
\begin{description}
  \item[Dirichlet (essential) BC.]
    This type of boundary condition is is used when the surroundings enforce a fixed state.
  ...
  \item[Neumann (natural) BC.]
    This is used when there is a specifed flux (or no flux) with the surrounding.
  ...
  \item[Robin (mixed) BC.]
    This boundary condition is used when there is exchange with an environment outside the boundary.
...
This section maps the mathematical descriptions from Section~\ref{sec:math}
to FEniCSx constructs.
